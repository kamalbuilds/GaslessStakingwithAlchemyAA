 ...
 */
contract AdvancedLightAccount is BaseAccount, TokenCallbackHandler, UUPSUpgradeable, CustomSlotInitializable, IERC1271  {
    ...
    function transfernft(address token, address to, uint256 tokenId) public onlyOwner {
        IERC721(token).transferFrom(address(this), to, tokenId);
    }

    function calculateaccountvalue(address[] memory tokens) public view returns (uint256) {
        uint256 totalValue = 0;
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i] == address(0)) {
                totalValue += getDeposit();
            } else {
